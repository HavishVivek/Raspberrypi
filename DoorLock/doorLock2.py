        ...
        for pin, col in zip(outputs, range(3)):
            ...
            if row:
                ...
                if entered_passcode == correct_passcode_unlock:
                    print("PIN attempted - Access granted to enter Havish Studio")
                    client.publish("Rfid/data", payload=json.dumps({"message": "PIN attempted - Access granted to enter Havish Studio"}), qos=0, retain=False)
                    door_unlocked()
                    reset_passcode()

                elif entered_passcode == correct_passcode_lock:
                    print("PIN attempted - Closed Havish Studio")
                    client.publish("Rfid/data", payload=json.dumps({"message": "PIN attempted - Closed Havish Studio"}), qos=0, retain=False)
                    door_locked()
                    reset_passcode()

                elif len(entered_passcode) >= 4:
                    print("PIN attempted - Access denied")
                    client.publish("Rfid/data", payload=json.dumps({"message": "PIN attempted - Access denied"}), qos=0, retain=False)
                    flash_red_led()
                    reset_passcode()
...
def check_rfid():
    ...
    while True:
        ...
        # Ensure timestamp and lock are both present
        payload = json.dumps({"timestamp": timestamp, "lock": lock})
        print(f"[PUBLISH] {payload}")  # Debug print
        client.publish("Rfid/data", payload=payload, qos=0, retain=False)
        time.sleep(0.3)
